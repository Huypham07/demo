...
\begin{itemize}
    \item Entailment ($\text{support}=1$): $\text{WRS} = P \times (1 - \lambda)$ \quad --- bằng chứng xác nhận, rủi ro giảm
    \item Neutral / không có bằng chứng ($\text{support}=0$): $\text{WRS} = P$ \quad --- rủi ro cơ sở
    \item Contradiction ($\text{support}=0$, khuếch đại): $\text{WRS} = P \times C = P \times 1{,}8$ \quad --- bằng chứng mâu thuẫn, rủi ro tăng
\end{itemize}
