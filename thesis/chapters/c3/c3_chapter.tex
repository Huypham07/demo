...
\textbf{Tối ưu hóa thực nghiệm (Empirical Grid Search).} Các tham số $(P_{\text{action}}, \lambda, C)$ được xác định thông qua phương pháp grid search trực tiếp trên toàn bộ tập dữ liệu thực nghiệm. 

Mục tiêu của thuật toán là \textit{tối đa hóa độ phân tán} của điểm EWRI, biến EWRI thành một thước đo có \textit{sức mạnh phân biệt} cao nhất. Trong phân tích chỉ số, độ phân tán lớn giúp làm nổi bật sự khác biệt giữa các ngân hàng minh bạch và các ngân hàng có rủi ro cao.
...
\hline
Implemented & 0{,}02 & 1{,}00 \\
\hline
Planning & 0{,}15 & 0{,}85 \\
\hline
Indeterminate & 0{,}55 & 0{,}50 \\
\hline
\end{tabular}
\end{table}
...
\begin{equation}
    \text{support}_i =
    \begin{cases}
        1 & \text{nếu tuyên bố }i\text{ có bằng chứng liên kết và nhãn NLI} \neq \text{contradiction} \\
        0 & \text{ngược lại (không có bằng chứng hoặc bằng chứng mâu thuẫn)}
    \end{cases}
...
\begin{enumerate}
    ...
    \item \textit{Giới hạn trên}: $\max(\text{WRS}) = 0{,}55 \times 1{,}8 = 0{,}99 < 1{,}0$, đảm bảo WRS luôn nằm trong khoảng $[0,\, 1]$.
    \item \textit{Phân hóa rủi ro}: Hệ số $1{,}8$ đủ lớn để trừng phạt mạnh các tuyên bố sai lệch, giúp tăng độ phân tán của điểm EWRI.
\end{enumerate}

Kết quả là ba mức WRS có thứ tự rõ ràng:
\begin{itemize}
    \item Entailment: $\text{WRS} = P \times (1 - \lambda)$ \quad --- bằng chứng giảm rủi ro
    \item Không có bằng chứng: $\text{WRS} = P$ \quad --- rủi ro cơ sở
    \item Contradiction: $\text{WRS} = P \times 1{,}8$ \quad --- rủi ro khuếch đại
\end{itemize}
